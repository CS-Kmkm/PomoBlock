...
type SyncedEvent = { account_id: string; id: string; title: string; start_at: string; end_at: string };
type Task = {
  id: string;
  title: string;
...
type DayItemSelection = { kind: DayItemKind; id: string } | null;
...
interface UiState {
  ...
  routineStudio: Record<string, unknown>;
  settings: {
    page: string;
    workStart: string;
    workEnd: string;
...
async function invokeCommand<T = unknown>(name: string, payload: CommandPayload = {}): Promise<T> {
...
async function safeInvoke<T = unknown>(name: string, payload: CommandPayload = {}): Promise<T> {
  try {
...
async function safeInvokeWithFallback<T = unknown>(
  primaryName: string,
...
async function invokeCommandWithProgress<T = unknown>(
  name: string,
  payload: CommandPayload = {}
): Promise<T> {
