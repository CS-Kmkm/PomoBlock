...
- [ ] 8. チェックポイント - ブロック生成のテスト
  - すべてのテストが通ることを確認し、質問があればユーザーに確認する

- [ ] 9. ポモドーロタイマーの実装
  - [ ] 9.1 PomodoroTimerの実装
    - タイマー状態管理（start, pause, resume, complete）
    - フェーズ遷移（Focus → Break）
    - _要件: 5.1, 5.3, 5.4, 5.5_
  
  - [ ] 9.2 PomodoroLogRepositoryの実装
    - ポモドーロ実績のローカル保存/読み込み
    - _要件: 5.5_
...
- [ ] 11. Git Repository管理の実装
  - [ ] 11.1 GitRepositoryの実装
    - libgit2を使用したGit操作（init, pull, commit, push）
    - ファイル読み書き
    - _要件: 9.1, 9.2, 9.3, 10.1_
  
  - [ ] 11.2 RoutineManagerの実装
...
- [ ] 12. チェックポイント - バックエンドコア機能のテスト
  - すべてのテストが通ることを確認し、質問があればユーザーに確認する

- [ ] 13. ローカルストレージリポジトリの実装
  - [ ] 13.1 LocalStorageRepositoryの実装
    - ブロック、タスク、ポモドーロログ、同期状態の保存/読み込み
    - SQLiteを使用したCRUD操作
    - _要件: 2.4, 2.5, 5.5, 11.1, 11.2_
  
  - [ ] 13.2 ローカルストレージのプロパティテスト
    - **プロパティ30: データ削除の完全性**
    - **検証: 要件 11.5**

- [ ] 14. ブロック操作の実装
  - [ ] 14.1 ブロック承認、削除、時刻調整の実装
    - 確定度更新とカレンダー反映
    - _要件: 4.2, 4.3, 4.4_
  
  - [ ] 14.2 ブロック操作のプロパティテスト
...
- [ ] 17. チェックポイント - バックエンド全機能のテスト
  - すべてのテストが通ることを確認し、質問があればユーザーに確認する

- [ ] 18. Tauri Command Handlersの実装
  - [ ] 18.1 認証・同期コマンドの実装
