...
- `src-ui/app-runtime.ts` (~2431 lines): ルーティング/初期化/state/共通ロジック/一部画面補助
...
- Today/Calendar/Now の共通補助をモジュール単位で追加分離する。

2. 型安全性の改善（継続）
- `app-runtime.ts` に `Unsafe` が残存。`DayBlockDragState` 周辺と command 応答型を段階的に明示型へ置換する。
- `routines-events.ts` の payload / map 処理の追加型付けを進める。
...
2. `Unsafe` が多い関数（`mockInvoke`, `refreshCoreData`, day drag系）から優先して型を明示化。
3. 文言互換（日本語）を確認し必要なら差し戻し。
