...
本ドキュメントは `doc/v2/log/routine_studio_refactor_fix_points.md` を実装タスクへ分解したもの。
...
| RSR-010 | 回帰確認（最小E2E） | 追加/削除/並び替え/Undo/Redo/保存/今日適用を確認するテストまたは手動検証手順を追加（`doc/v2/log/routine_studio_regression_checklist.md`） | 上記の主要フローが再現可能で、分割後も動作維持を確認できる | RSR-004,RSR-005,RSR-007,RSR-008 | P0 | 1日 |
...
- RSR-010: 完了（`doc/v2/log/routine_studio_regression_checklist.md` を追加）
